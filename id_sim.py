...
class IDSimNet(nn.Module):
    def __init__(self):
        ...
        ckpt_path = "./files/CurricularFace_Backbone.pth"
    ...
    def forward(self, img1, img2):
        assert (isinstance(img1, Image.Image) or isinstance(img1, str)) and (
                    isinstance(img2, Image.Image) or isinstance(img2,
                                                                str)), "img1 and img2 must be PIL.Image.Image or str"
        ...
        try:
            ...
            if img1 is None:
                print(f"Warning: Failed to detect and align face in image: {img1}")
                return torch.tensor(0.0) 
            id1 = self.facenet(self.transform(img1).unsqueeze(0).cuda())[0]
        except Exception as e:
            print(f"Error processing image: {img1}. Skipping. Error: {e}")
            return torch.tensor(0.0)  
        try:
            ...
            if img2 is None:
                print(f"Warning: Failed to detect and align face in image: {img2}")
                return torch.tensor(0.0)  
            id2 = self.facenet(self.transform(img2).unsqueeze(0).cuda())[0]
        except Exception as e:
            print(f"Error processing image: {img2}. Skipping. Error: {e}")
            return torch.tensor(0.0)  
